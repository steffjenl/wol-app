(() => {
    ...
    async function apiPost(action, deviceId) {
        const res = await fetch('/api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action, device_id: deviceId, csrf_token: window.CSRF_TOKEN }),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
    }

    function createDeviceController(card) {
        ...
        function setStatus(online) {
            isOnline = online;
            statusBadge.className = 'status-badge js-status-badge ' + (isWaking ? 'waking' : (online ? 'online' : 'offline'));
        ...
        async function pollStatus() {
            try {
                const data = await apiPost('status', deviceId);
                const online = !!data.online;

                if (isWaking && online) {
                    isWaking = false;
                    showToast(`${deviceName} is online!`, 'success');
                }
        ...
        wakeBtn.addEventListener('click', async () => {
            ...
            try {
                const data = await apiPost('wake', deviceId);
                if (data.success) {
                    isWaking = true;
                    setStatus(false);
                    showToast(`Magic packet voor ${deviceName} verzonden!`, 'success');
            ...
            wakeBtn.innerHTML = wakeButtonLabel;
        });
    ...
    document.querySelectorAll('.device-card[data-device-id]').forEach(createDeviceController);
})();
